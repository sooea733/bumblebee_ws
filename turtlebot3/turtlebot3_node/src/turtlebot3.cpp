...
void TurtleBot3::init_dynamixel_sdk_wrapper(const std::string & usb_port)
{
  ...
  dxl_sdk_wrapper_->init_read_memory(
    ...
    (extern_control_table.lift_value.addr - extern_control_table.millis.addr) +
    extern_control_table.lift_value.length
  );
}
...
void TurtleBot3::lift_callback()
{
  lift_control_sub_ = this->create_subscription<bumblebee_interfaces::msg::LiftControl>(
    "lift_control_topic",
    10,  // 기본 QoS depth 설정
    [this](const bumblebee_interfaces::msg::LiftControl::SharedPtr msg) -> void
    {
      std::string sdk_msg;
      ...
      lift.data.flag = msg->flag;
      lift.data.value = msg->value;

      const uint16_t start_addr = extern_control_table.lift_flag.addr;
      const uint16_t length = (extern_control_table.lift_value.addr - 
        extern_control_table.lift_flag.addr) + 
        extern_control_table.lift_flag.length;  // flag(4) + value(4)

      dxl_sdk_wrapper_->set_data_to_device(start_addr, length, lift.bytes, &sdk_msg);

      RCLCPP_INFO(
        this->get_logger(),
        "리프트 제어: flag=%d, value=%f, msg: %s",
        msg->flag, msg->value, sdk_msg.c_str());
    }
  );
}
