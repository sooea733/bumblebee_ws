...
DxlError Dynamixel::SetOperatingMode(uint8_t dxl_id, uint8_t dynamixel_mode)
{
  ...
  } else if(dynamixel_mode == DXL_EXTENDED_CTRL_MODE){
    fprintf(stderr, "Extended Control Mode\n");
  }
  else {
...
DxlError Dynamixel::SetSyncReadHandler(std::vector<uint8_t> id_arr)
{
  ...
  group_sync_read_ =
    ...
    IN_ADDR, indirect_info_read_[id_arr.at(0)].size); // 원래 fast였음
  ...
  for (auto it_read_data : read_data_list_) {
    ...
    for (size_t item_index = 0; item_index < indirect_info_read_[ID].cnt; item_index++) {
      ...
      if (indirect_info_read_[ID].item_name.at(item_index) == "Present Position") {
        double position_value = dxl_info_.ConvertValueToRadian(ID, static_cast<int32_t>(dxl_getdata));

        // 🔹 ID 12 모터의 경우 1/4로 변환
        if (ID == 12) {
          position_value /= 16.0;
         // printf("[Sync Read] Applying gear ratio (1/4) to ID 12: %.2f\n", position_value);
        }

        *it_read_data.item_data_ptr_vec.at(item_index) = position_value;
...
DxlError Dynamixel::SetBulkReadHandler(std::vector<uint8_t> id_arr)
{
  ...
  group_bulk_read_ = new dynamixel::GroupBulkRead(port_handler_, packet_handler_); //원래 FAST였음
  ...
  for (auto it_read_data : read_data_list_) {
    ...
    for (size_t item_index = 0; item_index < indirect_info_read_[ID].cnt; item_index++) {
      ...
      if (indirect_info_read_[ID].item_name.at(item_index) == "Present Position") {
        ...
        if (ID == 12) {
          position_value /= 16.0;
         // printf("[Bulk Read] Applying gear ratio (1/4) to ID 12: %.2f\n", position_value);
        }
  ...
  for (auto it_write_data : write_data_list_) {
    ...
    for (uint16_t item_index = 0; item_index < indirect_info_write_[ID].cnt; item_index++) {
      ...
      if (indirect_info_write_[ID].item_name.at(item_index) == "Goal Position") {
        ...
        if (ID == 12) {
          data *= 16.0;
        //  printf("[Sync Write] Applying gear ratio (×4) to ID 12: %.2f\n", data);
        }
    ...
    delete[] param_write_value;  // 동적 할당된 메모리 해제
  }
  ...
  for (auto it_write_data : write_data_list_) {
    ...
    for (uint16_t item_index = 0; item_index < indirect_info_write_[ID].cnt; item_index++) {
      ...
      if (indirect_info_write_[ID].item_name.at(item_index) == "Goal Position") {
        ...
        if (ID == 12) {
          data *= 16.0;
          //printf("[Bulk Write] Applying gear ratio (×4) to ID 12: %.2f\n", data);
        }
